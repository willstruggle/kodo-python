...
#include <kodo/perpetual/offset/random_sequence.hpp>
...
namespace offset
{
struct random_sequence_wrapper : kodo::perpetual::offset::random_sequence
{
    std::function<void(const std::string&)> m_log_callback;
};

using random_sequence_type = random_sequence_wrapper;

void random_sequence_enable_log(
    random_sequence_type& offset_generator,
    std::function<void(const std::string&)> callback)
{
    ...
    offset_generator.enable_log(
        [](const std::string& message, void* data) {
            random_sequence_type* offset_generator =
                static_cast<random_sequence_type*>(data);
...
void random_sequence(pybind11::module m)
{
    using namespace pybind11;
    class_<random_sequence_type>(
        m, "RandomSequence",
        "Generates a sequential random offset for the perpetual code.")
        .def(init<>(), "The random_sequence constructor.\n")
        .def("configure", &random_sequence_type::configure, arg("symbols"),
             arg("width"),
             ...
             "coefficients.\n")
        .def(
            "reset", &random_sequence_type::reset,
            "Resets the generator to the state when it was first configured.\n")
        .def_property_readonly(
            "symbols", &random_sequence_type::symbols,
            "Return the number of symbols supported by this generator.\n")
        .def_property_readonly(
            "width", &random_sequence_type::width,
            "Return the width of the generator in symbols.\n")
        .def("offset", &random_sequence_type::offset,
             "Return the next offset.\n")
        .def("enable_log", &random_sequence_enable_log, arg("callback"),
             "Enable logging for the generator.\n\n"
             "\t:param callback: The callback used for handling messages.\n")
        .def("disable_log", &random_sequence_type::disable_log,
             "Disables the log.\n")
        .def("is_log_enabled", &random_sequence_type::is_log_enabled,
